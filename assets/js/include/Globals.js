...
let setContainersActive;
let randomRange;
let shuffleArray;
let loadFile;
let setUIDisplayLang;
let CreateToast;
let CreateStaticNotification
let parseCSV;
let parseXLSX;
let TopProgressBar;
let getLoginSession;
...
checkPermission = (perm) =>
{
	...
	// 	|| USER_CONFIG.LOGIN_TYPE == EMP_TYPE_DEPUTY_GENERAL_MANAGER ) return true

	if ( isNull(perm) ) return false

	// granted
	for (let i = 0; i < PERMISSIONS.granted.length; i++) 
	{
		const granted = PERMISSIONS.granted[i]
...
setupPermissions = () =>
{
	...
	if ( USER_CONFIG.hasAllPermissions )
	{
		// var perm_elements = $('[data-perm]')
		// all
		for (let i = 0; i < perm_elements.length; i++) 
		{
			...
			if ( element.data('perm-attribute') == 'show_to_granted_only' )
			{
...
getMyPermissions = async () =>
{
	...
	// var response = await EMPLOYEE_MODEL.type_permission_self()
	...
	{
		EmployeePrivilegesWorker.onmessage = (e) =>
		{
...
isPDFFile = (url) =>
{
	...
	return ext.includes( extractFileExtension(url).toLowerCase() );
}
// isMSPowerPointFile
isMSPowerPointFile = (url) =>
{
	var ext = ['ppt', 'pptx'];
...
isVideoFile = (url) =>
{
	var ext = ['mp4', 'mov', 'mkv', 'flv', '3gp', 'wav'];
...
isImageFile = (url) =>
{
	...
	return ext.includes( (extractExtensionFromDataUrl(url) ) ?? extractFileExtension(url).toLowerCase() );
}
// set Patient Dept
...
{
	var url = API_END_POINT+'TreatmentClasses/batchDelete';
	var data = {
		list: list
	}
...
deleteExpenses = (list) =>
{
	var url = API_END_POINT+'Treasury/Expenses/deleteList';
	var data = {
		list: list
	};
...
{
	var url = API_END_POINT+'Treasury/Expenses/filterBetweenDates';
	var data = {
...
{
	var url = API_END_POINT+'Treasury/Expenses/filterByDate';
	var data = {
		TreasuryObject: TreasuryObject
	};
...
{
	var url = API_END_POINT+'Treasury/Expenses/list';
	var data = {
		clinicId: clinicId
	};
...
{
	var url = API_END_POINT+'Employees/Types/index';
	var data = {
...
// download file
downloadFile = async (url, filename, progressInfo, onComplete) =>
{
	var DOWNLOAD_START_TIME = undefined;
	const ufs = require("url-file-size")

	const size = await ufs(url)
	...
	var request = $.ajax({
		xhr: function() 
		{
		  var xhr = new XMLHttpRequest();
			...
			{
			    var percentComplete = (e.loaded / size) * 100;
				// Time Remaining
...
// notification
CreateStaticNotification = (options = {}) =>
{
	const defaultOptions = {
		parent: $('.static-notification-container'),
		html: '',
		class: 'is-warning'
	}

	var allOptions = {...defaultOptions, ...options}
	var containerHTML = `<div class="notification ${allOptions.class} mb-2">
							<div class="has-text-aligned-locale-inverted">
								<button class="delete notification-close"></button>
							</div>
							${allOptions.html}
						</div>`
	if ( !allOptions.parent[0] )
	{
		$(`<div class="static-notification-container my-3">${containerHTML}</div>`).insertAfter(MAIN_CONTENT_CONTAINER.find('h1'))
		return
	}
	allOptions.parent.append(containerHTML)
}
// Toast
CreateToast = (title = '', body = '', time = 'À présent', delay = 10000) =>
{
	...
	var toastHTML = `<div class="${tclass} toast" role="alert" aria-live="polite" aria-atomic="true" data-delay="${delay}">
						...
						<div class="toast-body fs-17">
							${body}
						</div>
						<audio src="${DataURI.TOAST_TONE}" autoplay class="d-none"></audio>
					</div>`;
...
// setContainersActive
setContainersActive = (disabled = true) =>
{
	if ( disabled )
	{
...
loadNavbarLinks = () =>
{
	...
	links.push( TOP_NAV_BAR.find('[data-role="NAV_LINK"]') )
...
{
	var previous_page = JSON.parse(sessionStorage.getItem('previous_page'))

	var backButtonCSS = `position: absolute;
						top: 20px;
						left: 30px;
						z-index:5;`
	if ( FUI_DISPLAY_LANG.lang == 'fr' )
	{
		backButtonCSS = `position: absolute;
						top: 20px;
						right: 30px;
						z-index:5;`
	}
	var backButtonHTML = `<div class="my-2" style="${backButtonCSS}">
								<button class="btn btn-success btn-sm shadow" id="page_back_button">
									${FUI_DISPLAY_LANG.views.pages.global.back_button}
								</button>
							</div>`
	getPage(previous_page.link).then(response =>
	{
		MAIN_CONTENT_CONTAINER.html(response);
		toggleSimilarNavbarsLinks(previous_page.link)
		if (data)
		...
		for (let i = 0; i < links.length; i++) {
			const nav_link = SIDE_NAV_CONTAINER.find(links[i])
			
			if ( nav_link.data('name') == previous_page.name )
			{
				var nav_text = nav_link.text()
				var nav_parent = nav_link.closest('.list-dropdown-toggle')
				if ( nav_parent[0] )
					nav_text = `${nav_parent.find('a').first().text()} - ${nav_text}`

				MAIN_CONTENT_CONTAINER.find('h1').text(nav_text).parent().removeClass('d-none')
				break
			}
		}

		// store current page
    	sessionStorage.setItem('current_page', sessionStorage.getItem('previous_page'));
		// retrieve previous session data
		if ( previousSessionData() )
		{
			sessionStorage.setItem('session', JSON.stringify(previousSessionData()) )
		}
		// remove old back button
		// var old_back_button = $('.back-button')
		// if ( old_back_button[0] ) old_back_button.remove()
		// append back button
		var back_button = MAIN_CONTENT_CONTAINER.find('#page_back_button')
		if ( !back_button[0] )
		{
			MAIN_CONTENT_CONTAINER.prepend(backButtonHTML)
			back_button = MAIN_CONTENT_CONTAINER.find('#page_back_button')
		}
		// set button css if already exists
		back_button.parent().attr('style', backButtonCSS)
			
		// previous page button
		back_button.off('click').on('click', e =>
		{
			goToPreviousPage()
		})
...
goToPage = (name, data) =>
{
	...
	getPage(page).then(response =>
	{
		...
		// store current page
    	sessionStorage.setItem('current_page', JSON.stringify({
			link: page,
			name: name,
		}));
		MAIN_CONTENT_CONTAINER.html(response);
		...
		if (data)
		{
			...
			{
				sessionStorage.setItem('session', JSON.stringify(data) )
				sessionStorage.setItem('previous_session', JSON.stringify(data) )
			}
		}
...
// previous session
previousSessionData = (clear_after_retrieve = false) =>
{
	var data = {}
	try {
		data = JSON.parse( sessionStorage.getItem('previous_session') )
	} catch (error) {
		return data
	}

	if ( clear_after_retrieve )
		sessionStorage.removeItem('previous_session')
...
try {
	...
	dispatchCustomEvent('user-interface-setup-has-finished')
}
dispatchCustomEvent('user-interface-setup-has-finished')
